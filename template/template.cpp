...
void main()
{
	...
	app = new Game();
	app->screen = screen;
	fSurface* fscreen = new fSurface(SCRWIDTH, SCRHEIGHT);
	app->fscreen = fscreen;
	fSurface* lightOnlySurface = new fSurface(SCRWIDTH, SCRHEIGHT);
	app->lightOnlySurface = lightOnlySurface;
	app->Init();
	...
	while (!glfwWindowShouldClose( window ))
	{
		...
		if (frameNr++ > 1)
		{
			//if (app->fscreen) renderTarget->CopyFrom( app->fscreen );
			fscreen->CopyTo(screen, 0, 0);
