...
// float3 equivalents
void GLTexture::CopyFrom(fSurface* src)
{
	glBindTexture(GL_TEXTURE_2D, ID);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_FLOAT, src->pixels);
	CheckGL();
}

void GLTexture::CopyTo(fSurface* dst)
{
	glBindTexture(GL_TEXTURE_2D, ID);
	glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_FLOAT, dst->pixels);
	CheckGL();
}
